...
namespace TSQL
{
    public partial class TSQLStatementReader
    {
        ...
        public static TSQLStatement ParseStatement(string tsqlText,
            bool useQuotedIdentifiers = false,
            bool includeWhitespace = false) => ParseStatements(tsqlText, useQuotedIdentifiers, includeWhitespace).FirstOrDefault();
        ...
        public bool MoveNext()
        {
            ...
            if (hasMore)
            {
                // eat up any tokens inbetween statements until we get to something that might start
                // a new statement which should be a keyword if the batch is valid

                // if the last statement parser did not swallow the final semicolon, or there were
                // multiple semicolons, we will swallow it also
                while (
