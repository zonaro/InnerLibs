...
namespace InnerLibs
{
    ...
    public class FullMoneyWriter : FullNumberWriter
    {
        ...
        public override string ToString(decimal Number, int DecimalPlaces = 2)
        {
            ...
            return (InExtensive(num) + CurrencyCentsName[num].Wrap(" ") + (dec == 0L | DecimalPlaces == 0 ? InnerLibs.Util.Empty : And.Wrap(" ") + InExtensive(dec) + CurrencyCentsName[dec].Wrap(" "))).ToLowerInvariant().TrimBetween();
        }
