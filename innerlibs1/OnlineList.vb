...
Namespace Online
    ...
    Public Class OnlineList(Of UserType As Class, IdType As Structure)
        ...
        ''' <summary>
        ''' Cira uma nova instancia de OnlineList apontando a propriedade do ID do usuario
        ''' </summary>
        ...
        ''' </summary>
        ''' <returns></returns>
        Public ReadOnly Property Chat As New UserChat(Of UserType, IdType)(Function(x) idgetter(x), Me)
        ...
        ''' <summary>
        ''' Retorna um OnlineUser a partir de uma instancia de <see cref="UserType"/>
        ''' </summary>
        ...
        Public Function OfflineUsers() As IEnumerable(Of OnlineUser(Of UserType, IdType))
            Return Me.Where(Function(x) x.Value.IsOnline = False).Select(Function(x) x.Value)
        End Function
        ...
        ''' <returns></returns>
        Public Function GetUsers(Optional IsOnline As Boolean? = Nothing) As IEnumerable(Of UserType)
            Return Me.Where(Function(x) x.Value.IsOnline = If(IsOnline, x.Value.IsOnline)).Select(Function(x) x.Value.Data)
        End Function
